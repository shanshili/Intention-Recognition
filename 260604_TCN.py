...
def main():
    ...
    # 构建模型（传入target_idx, 使用 AttentionPooling + 3层TCN）
    model = IntentModel(
        N=N, node_feat_dim=1, spat_hid=64, spat_out=32,
        temp_hid=64, intent_dim=INTENT_DIM, num_classes=3,
        pred_len=PRED_LEN, use_tcn=USE_TCN,
        tcn_channels=(64, 128, 256),
        target_idx=target_idx,
        use_attention_pooling=True, predictor_layers=3,
    )
    ...
    (train_losses, val_losses, train_cls, val_cls, train_pred, val_pred,
     val_accuracies, best_model_path) = train_model(
        ...
        patience=EARLY_STOP_PATIENCE, use_adaptive_loss=False,
        weight_cls=WEIGHT_CLS, weight_pred=WEIGHT_PRED,
    )
