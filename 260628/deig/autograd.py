...
Supported ops: +, -, *, matmul, sum, mean, relu, leaky_relu, sigmoid, tanh,
exp, log, gather(rows), concat(axis=1), scatter_add (segment sum),
segment_softmax, softmax_lastdim, and a Huber loss helper. This is enough to
build a fully-differentiable GAT spatial encoder and a dilated-causal TCN
temporal encoder (see gat_tcn.py), so the prediction loss back-propagates
through the WHOLE GAT-TCN stack -- not just the edge head.
"""
...
class Tensor:
    ...
    def leaky_relu(self, alpha: float = 0.2):
...
def softmax_lastdim(x: Tensor) -> Tensor:
    """Numerically-stable softmax over the last axis of a 2-D tensor.

    Used for TCN temporal attention pooling: beta_{i,tau} over the W time
    steps of each node. Backward uses the standard softmax Jacobian-vector
    product  dx = p * (g - sum(g*p)).
    """
    ...
    def _backward():
        if x.requires_grad:
            x._ensure_grad()
            g = out.grad
            x.grad += p * (g - (g * p).sum(axis=-1, keepdims=True))
    out._backward = _backward
    return out
