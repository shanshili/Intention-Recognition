"""
DEIG-TCN trainable model (Sections 6-9, 12-13).

CHANGE LOG (why this version "actually uses GAT-TCN")
----------------------------------------------------
Previously the GAT and TCN were realised as a *fixed* feature map in
features.py, and only the edge head + predictor were trained. Here the node
window representation Z is produced by a **trainable** GAT-TCN encoder
(gat_tcn.STEncoder) that is part of the autograd graph, so the prediction loss
back-propagates through:

    GAT spatial attention  ->  TCN temporal conv  ->  temporal pooling
        ->  edge-intent head  ->  intent-weighted message passing  ->  predictor

Trainable components:
  * STEncoder      : GAT(L_g layers) + dilated-causal TCN + temporal attention
                     -> per-node window vector z_i^t                 (Sec 6)
  * EdgeIntentHead : MLP(m_ij^t) -> sigmoid -> s_ij in [0,1]         (Sec 7)
  * W_m            : message projection for intent-weighted propagation
  * Predictor      : MLP([z_j || g_j]) -> H-step residual forecast   (Sec 8)
...
from .autograd import Tensor, concat, huber_loss, parameter, _safe_div
from .gat_tcn import STEncoder
...
class DEIG_TCN:
    def __init__(self, F0, Fedge, H, W, f_gat=24, f_tcn=24, gat_layers=2,
                 hidden=32, msg_dim=16, seed=0):
        self.H = H
        self.W = W
        # ----- trainable GAT-TCN spatio-temporal encoder (Sec 6) -----
        self.encoder = STEncoder(F0=F0, f_gat=f_gat, f_tcn=f_tcn,
                                 f_edge=Fedge, W=W, gat_layers=gat_layers,
                                 seed=seed)
        Fz = self.encoder.Fz                 # encoder output dim = z_i^t dim
        self.Fz = Fz
        # ----- edge intent head (Sec 7) -----
        Fm = 4 * Fz + Fedge                  # [z_i,z_j,z_i-z_j,z_i*z_j, edge_block]
        self.W1 = parameter((Fm, hidden), seed=seed + 100)
        self.b1 = Tensor(np.zeros(hidden), requires_grad=True)
        self.W2 = parameter((hidden, 1), seed=seed + 101)
        self.b2 = Tensor(np.zeros(1), requires_grad=True)
        # ----- message projection -----
        self.Wm = parameter((Fz, msg_dim), seed=seed + 102)
        # ----- predictor (Sec 8) -----
        Fp = Fz + msg_dim
        self.Wp1 = parameter((Fp, hidden), seed=seed + 103)
        self.bp1 = Tensor(np.zeros(hidden), requires_grad=True)
        self.Wp2 = parameter((hidden, H), seed=seed + 104)
        self.bp2 = Tensor(np.zeros(H), requires_grad=True)

    def params(self):
        return (self.encoder.params()
                + [self.W1, self.b1, self.W2, self.b2, self.Wm,
                   self.Wp1, self.bp1, self.Wp2, self.bp2])

    # ---- encoder ---------------------------------------------------------
    def encode(self, batch) -> Tensor:
        """Run the trainable GAT-TCN encoder -> Z (BN, Fz)."""
        F_seq = [Tensor(F_tau) for F_tau in batch["F_seq_list"]]   # W x (BN,F0)
        edge_block = Tensor(batch["edge_block"])
        return self.encoder(F_seq, batch["src"], batch["dst"],
                            edge_block, batch["num_nodes"])
...
def assemble_batch(window_inputs, targets, cand, reverse_index):
    ...
    window_inputs : list of dicts from features.build_window_inputs, each with
                    F_seq (W, N, F0), edge_block (E, Fe), z_last (N,)
    targets       : list of (N, H) arrays
    reverse_index : array mapping edge e -> reverse edge index (or -1)
    """
    B = len(window_inputs)
    F_seq0 = window_inputs[0]["F_seq"]
    W, N, F0 = F_seq0.shape
    E = cand["E"]
    src0, dst0 = cand["src"], cand["dst"]

    # per-time-step batched node features: list of W arrays, each (B*N, F0)
    F_seq_list = [np.concatenate([w["F_seq"][tau] for w in window_inputs],
                                 axis=0) for tau in range(W)]
    ...
    for e in range(E):
        ...
        if r >= 0 and e < r and (e, r) not in seen:
            ...
            for b in range(B):
                ...
                pb.append(r + b * E)
    return {
        "F_seq_list": F_seq_list, "edge_block": edge_block,
        "z_last": z_last, "target": target,
