...
def run(cfg: Config, intent_step: int):
    ...
    # 只补充严格时间对齐，不改变任何绘图参数或视觉逻辑。
    # step 是意图局部索引 c，必须先映射到原始流量位置 pos。
    intent_positions = np.asarray(bundle["intent_positions"], dtype=int)
    timestamps = pd.DatetimeIndex(bundle["timestamps"])
    pos = int(intent_positions[step])
    dt_str = timestamps[pos].strftime("%Y-%m-%d %H:%M:%S")
    print(f"[viz-inputs] c={step} -> pos={pos} -> {dt_str}")

    p1 = plot_causal_graph(causal, coords, paths, dt_str=dt_str)
